...
function CompletionDot({ status }: { status: string }) {
  if (status === "pending_review") {
    return <span className="w-2 h-2 rounded-full bg-loss block shrink-0 animate-pulse" title="待复盘" />;
  }
  if (status === "pending_improve") {
    return <span className="w-2 h-2 rounded-full bg-warning block shrink-0" title="待完善" />;
  }
  if (status === "complete") {
    return <span className="text-[10px] text-profit leading-none shrink-0">✓</span>;
  }
  return <span className="w-2 h-2 rounded-full bg-border block shrink-0" title="未开始" />;
}

// ─── Month Stats ──────────────────────────────────────────────────────────────

function calcMonthStats(bets: BetRecord[]) {
  let totalBet = 0;
  let totalPnl = 0;
  for (const r of bets) {
...
export default function RecordsPage() {
  ...
  const stats = useMemo(() => calcMonthStats(monthBets), [monthBets]);
    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <div className="sticky top-0 z-20 bg-background border-b border-border">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <span className="font-semibold text-sm">记录</span>
        </div>
          ...
          {r.result?.errors && r.result.errors.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {r.result.errors.slice(0, 3).map((e) => (
                <span key={e} className="text-[9px] px-1.5 py-0.5 rounded bg-loss/10 text-loss border border-loss/20">
                  {e}
                </span>
              ))}
            </div>
